...
namespace ego_planner
{
                                              ...
                                              double &costp)
  {
    ...
    for (size_t id = 0; id < size; id++)
    {
      ...
      double traj_i_satrt_time = swarm_trajs_->at(id).start_time.seconds();
  ...
  {
    rclcpp::Parameter cps_num_prePiece_p, wei_obs_p, wei_swarm_p,\
                        wei_feas_p, wei_sqrvar_p, wei_time_p, wei_formation_p,\
                        obs_clearance_p, swarm_clearance_p, formation_type_p, max_vel_p, max_acc_p;
    node_->get_parameter_or("optimization/constrain_points_perPiece", cps_num_prePiece_p, rclcpp::Parameter("optimization/constrain_points_perPiece", -1));
    node_->get_parameter_or("optimization/weight_obstacle", wei_obs_p, rclcpp::Parameter("optimization/weight_obstacle", -1.0));
    node_->get_parameter_or("optimization/weight_swarm", wei_swarm_p, rclcpp::Parameter("optimization/weight_swarm", -1.0));
    node_->get_parameter_or("optimization/weight_feasibility", wei_feas_p, rclcpp::Parameter("optimization/weight_feasibility", -1.0));
    node_->get_parameter_or("optimization/weight_sqrvariance", wei_sqrvar_p, rclcpp::Parameter("optimization/weight_sqrvariance", -1.0));
    node_->get_parameter_or("optimization/weight_time", wei_time_p, rclcpp::Parameter("optimization/weight_time", -1.0));
    node_->get_parameter_or("optimization/weight_formation", wei_formation_p, rclcpp::Parameter("optimization/weight_formation", -1.0));

    node_->get_parameter_or("optimization/obstacle_clearance", obs_clearance_p, rclcpp::Parameter("optimization/obstacle_clearance", -1.0));
    node_->get_parameter_or("optimization/swarm_clearance", swarm_clearance_p, rclcpp::Parameter("optimization/swarm_clearance", -1.0));
    node_->get_parameter_or("optimization/formation_type", formation_type_p, rclcpp::Parameter("optimization/formation_type", -1));
    node_->get_parameter_or("optimization/max_vel", max_vel_p, rclcpp::Parameter("optimization/max_vel", -1.0));
    node_->get_parameter_or("optimization/max_acc", max_acc_p, rclcpp::Parameter("optimization/max_acc", -1.0));

    cps_num_prePiece_ = cps_num_prePiece_p.as_int();
    wei_obs_ = wei_obs_p.as_double();
    wei_swarm_ = wei_swarm_p.as_double();
    wei_feas_ = wei_feas_p.as_double();
    wei_sqrvar_ = wei_sqrvar_p.as_double();
    wei_time_ = wei_time_p.as_double();
    wei_formation_ = wei_formation_p.as_double();
    obs_clearance_ = obs_clearance_p.as_double();
    swarm_clearance_ = swarm_clearance_p.as_double();
    formation_type_ = formation_type_p.as_int();
    max_vel_ = max_vel_p.as_double();
    max_acc_ = max_acc_p.as_double();
