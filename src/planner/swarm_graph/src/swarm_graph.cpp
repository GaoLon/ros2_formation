...
bool SwarmGraph::updateGraph( const std::vector<Eigen::Vector3d> &swarm){ 
    ...
    if(nodes.size() != nodes_des.size()){
        ...
        std::cout <<"Size of swarm formation vector is incorrect. \n";
        return false;
    }
    ...
    if( have_desired ){
        ...
        std::cout << "Please enter the desired formation!!\n" ;
    }
...
bool SwarmGraph::calcFNorm2( double &cost ){
    ...
    if( have_desired ){
        ...
        std::cout <<"Invalid desired formation.\n" ;
        return false;
    }
}
...
Eigen::Vector3d SwarmGraph::getGrad(int id){
    if (have_desired && id<nodes_des.size()){
        ...
        std::cout <<"id is error !!! or desired graph has not been setup !!!\n";
        return grad;
    }
...
bool SwarmGraph::getGrad(std::vector<Eigen::Vector3d> &swarm_grad){
    if (have_desired){
        ...
        std::cout <<"desired graph has not been setup !!!\n";
        return false;
    }
}
