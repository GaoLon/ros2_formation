...
bool AStar::AstarSearch(const double step_size, Vector3d start_pt, Vector3d end_pt, bool use_esdf_check)
{
    ...
    while (!openSet_.empty())
    {
        ...
        // ros::Time time_2 = ros::Time::now();
        // rclcpp::Time time_2 = nh->now();
        clock_t time_2 = clock();
        if ((float)(time_2 - time_1)/CLOCKS_PER_SEC > 0.2)
        {
    ...
    if ((float)(time_2 - time_1)/CLOCKS_PER_SEC > 0.1)
        printf("Time consume in A star path finding is %dms, iter=%d\n", (int)(time_2 - time_1), num_iter);
